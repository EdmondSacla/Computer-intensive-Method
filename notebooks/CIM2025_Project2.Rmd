...
$RH_0$, there is significant different between the two models.Therefore it is important to keep _good spine condition_ as a predictor of number of satellites.
...
- Test statistic: 
D=−2⋅(loglikelihood of reduce model−loglikelihood of full model)
-Theoretical distribution under $H_0$:\(D \sim \chi^2_1\)
We will now obtain the bootstrap distribution of 𝐷using parametric and non-parametric methods.
...
# Observed lrt
lrt_obs<-qchisq(0.95, df=1)
# P-values
p1<- (1 + sum(lrt_para > lrt_obs))/(1 + B) 
p2<-(1 + sum(lrt_nonpara > lrt_obs))/(1 + B)

#p2<-pchisq(mean(lrt_para), df = 1, lower.tail = FALSE)
#p1<-pchisq(mean(lrt_nonpara), df = 1, lower.tail = FALSE)
...
At a significance level of 5\%, only the parametric bootstrap seems to identify a significant effect of _good spine condition_.
...
```
The non-parametric bootstrap distributions deviate significantly theoretical \chi^2_1\ distribution, this suggest that the theoretical distribution does not fully capture the data's behavior under the null hypothesis.

The parametric bootstrap distributions aligns somehow the theoretical \chi^2_1\ distribution, this suggest that the theoretical distribution more or less y capture the data's behavior under the null hypothesis.
...
pp<- (1 + sum(lr.perm > teststat))/(1 + B) # p-value
sprintf("likelihood ratio test using permutation (p-value): %.3f", pp)
```

At a significance level of 5\%, only the permutations test does not identify a significant effect of _good spine condition_.
...
pt<-(1 + sum(abs(t.boot) > abs(t.obs)))/(1 + B)
...
# p-value
ptm<- (1 + sum(abs(tm.boot) > abs(tM.obs)))/(1 + B)
sprintf(" Parametric bootstrap for testing tm: %.3f", ptm)
```

p-value=0.638 ==> no significant difference between the two group of treatment.
...
sprintf("Jackknife non parametric bootstrap: %.30f", jack_se)
```
...
```{r}
quantile(theta.bs[[8]], probs = c(0.025, 0.975))
```
...
#hist(bootstrap_ratios, nclass = 50)
